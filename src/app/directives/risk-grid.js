(function() {
    ...
    angular.module("dashboard")
        .directive('riskGrid', function ($http, $localStorage, $sce, Paginator, $timeout, $interval, $httpParamSerializer, FileSaver) {
        return {
            scope:{
                id: "@?",
                tableCss: "@?",
            ...
            restrict: 'E',
            replace: 'true',
            templateUrl: "app/directives/risk-grid.html",
            link: function (scope, elem, attr) {
                ...
                scope.search = function() {
                    scope.config.datasource.params.query = scope.searchCase;
                    scope.config.datasource.params.page = 1;
                    queryREST();
                };
                
                scope.searchOnChange = function() {
                    if(scope.searchCase.length == 0) {
                      queryREST();
                    }
                };
                ...
                scope.checkEvent = function(id) {
                    if (scope.selectedItms.indexOf(id) !== -1) {
                        scope.selectedItms.splice(scope.selectedItms.indexOf(id), 1);
                    }else {
                        scope.selectedItms.push(id)
                    }
                    // console.log(scope.selectedItms)
                }
               ...
               scope.api.sendMessage = function(){
                   ...
                   if(scope.websocket!==undefined && scope.websocket.readyState==1){
                ...
                function queryREST(){
                    var resolve = new Promise(function(resolve, reject){
                        ...
                        .success(function(response, status, headers){
                            ...
                            if (scope.config.datasource.paramsOut.data === 'cases') {
                                scope.data = correctSummary(scope.data);
                            }
                ...
                function correctSummary(summary) {
                    var res = [];
